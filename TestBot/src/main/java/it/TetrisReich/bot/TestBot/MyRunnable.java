...
public class MyRunnable implements Runnable {
    ...
    public void run() {
    	...
    	while(true){
    				...
    				if(message.chat().id().toString().equals("50731050")||App.checkAdm(message.chat().id())){
    					...
    					if(sp[0].equalsIgnoreCase("/force")){
    						...
    						if(sp[1].equalsIgnoreCase("vUpdate")) {
    							...
    							//MISONOFERMATOQUIIIIIIIIIIIIIIIIIII
    					...
    					if(sp[0].equalsIgnoreCase("/file")){
    						...
    						if(sp[1].equalsIgnoreCase("read")){
    							...
    							if(parse){
            						bot.execute(new SendMessage(message.chat().id().toString(),App.aL(sp[2],true))
    		...
    		tesThread++; // SocketTimeoutException
			...
			//System.out.println(tesThread);ù
			//if(true) return;
	    	if(crash) crash();
    	}
    }
